...
class SmplxDeformer():
    def __init__(self, model_path="./data/body_models", gender='neutral', num_betas=300, use_pca=False):
        ...
        self.vposer.load_state_dict(torch.load(os.path.join(model_path, "TR00_E096.pt"), map_location='cpu'))
    ...
    def smplx_forward(self, smplx_param):
        if 'latent' in smplx_param:
            body_pose = pytorch3d.transforms.rotation_conversions.matrix_to_axis_angle(
                self.vposer.decode(smplx_param['latent']).view(-1, 3, 3)).view(smplx_param['latent'].shape[0], -1)
        else:
        ...
        if len(scale.shape):
            scale = scale.unsqueeze(dim=1).unsqueeze(dim=2)
    ...
    def smplx_forward_simple(self, smplx_param):
        ...
        smplx_out = self.smplx_model.forward(**smplx_param)

        full_pose = smplx_out.full_pose
        batch_size = full_pose.shape[0]
        J = lbs.vertices2joints(self.smplx_model.J_regressor, smplx_out.v_shaped)
        rot_mats = lbs.batch_rodrigues(full_pose.view(-1, 3)).view(batch_size, -1, 3, 3)
        _, transform_mat = lbs.batch_rigid_transform(rot_mats, J, self.smplx_model.parents)
        smplx_out.transform_mat = transform_mat
        return smplx_out
    ...
    def export_simple(self, smplx_param, save_path):
        smplx_out = self.smplx_forward_simple(smplx_param)
        with open(save_path, 'w') as smplx_save:
            for v in smplx_out.vertices[0].detach().cpu().numpy():
                smplx_save.write('v {} {} {}\n'.format(v[0], v[1], v[2]))
            for f in self.smplx_model.faces:
    ...
    def subdivide_mesh(self, iterations=0):
        v = self.smplx_model.v_template.cpu().numpy()
        faces = self.smplx_model.faces
        w = self.smplx_model.lbs_weights.cpu().numpy()
        v_index = np.argwhere(w[:, :3].sum(axis=1) > 0.5).squeeze()
        face_mask = np.isin(faces, v_index).all(axis=1)
        ...
        for _ in range(iterations):
            ...
            edges_list.append(edges[unique])
            
            mid_idx = inverse.reshape((-1, 3)) + num_v
            num_v += unique.shape[0]

            faces_subset = np.column_stack(
                [
                    faces_subset[:, 0],
                    mid_idx[:, 0],
                    mid_idx[:, 2],
        ...
        new_faces = np.vstack((faces[~face_mask], faces_subset))

        return new_faces, edges_list