...
class ActorsHQDataset(Dataset):
    ...
    def __init__(
        ...
        self.camera_path = os.path.join(data_dir, f"a{actor}_s{sequence}", "cam_info.json")
        self.image_path = os.path.join(data_dir, "ActorsHQ", f"Actor{actor:02d}", f"Sequence{sequence}", "4x")
    ...
    def _load_camera_dataset(self):
        ...
        cameras = []
        cam_centers = []
        for camera_id, cam_info in contents.items():
            w_raw, h_raw, k, c2w = cam_info['W'], cam_info['H'], cam_info['K'], np.array(cam_info['RT'])
