...
class ShadowUNet(nn.Module):
    def __init__(
        ...
        for i, size in enumerate(self.sizes):
            ...
            # print(f"EncoderLayers({i}): {n_in}, {n_out}, {size}")
        ...
        for i in range(len(self.sizes)):
            ...
            # print(f"DecoderLayer({i}): {n_in}, {n_out}, {size}")
...
class FloorShadowDecoder(nn.Module):
    def __init__(
        ...
        self.down1 = nn.Sequential(
            Conv2dWNUB(1, 64, 256, 256, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.down2 = nn.Sequential(
            Conv2dWNUB(64, 64, 128, 128, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.down3 = nn.Sequential(
            Conv2dWNUB(64, 128, 64, 64, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.down4 = nn.Sequential(
            Conv2dWNUB(128, 256, 32, 32, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.down5 = nn.Sequential(
            Conv2dWNUB(256, 512, 16, 16, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.up1 = nn.Sequential(
            ConvTranspose2dWNUB(512, 256, 32, 32, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.up2 = nn.Sequential(
            ConvTranspose2dWNUB(256, 128, 64, 64, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.up3 = nn.Sequential(
            ConvTranspose2dWNUB(128, 64, 128, 128, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.up4 = nn.Sequential(
            ConvTranspose2dWNUB(64, 64, 256, 256, 4, 2, 1), nn.LeakyReLU(0.2)
        )
        self.up5 = nn.Sequential(ConvTranspose2dWNUB(64, 1, 512, 512, 4, 2, 1))
...
class ShadowUNet_PoseCond(nn.Module):
    def __init__(
        ...
        # print(f" shadow map size: {self.shadow_size}")
        # print(f"sizes: {self.sizes}")
        ...
        self.pose_fc_block = nn.Sequential(
            LinearWN(self.num_pose_dims, self.num_pose_enc_dims),
            nn.LeakyReLU(lrelu_slope),
        )

        self.pose_conv_block = Conv2dWNUB(
...
class PoseToShadow(nn.Module):
    def __init__(
        ...
        self.fc_block = nn.Sequential(
            LinearWN(self.n_pose_dims, 256 * 4 * 4),
            nn.LeakyReLU(0.2),
        )
        self.conv_block = nn.Sequential(
            ConvTranspose2dWNUB(256, 256, 8, 8, 4, 2, 1),
            nn.LeakyReLU(0.2),
            ConvTranspose2dWNUB(256, 128, 16, 16, 4, 2, 1),
            nn.LeakyReLU(0.2),
            ConvTranspose2dWNUB(128, 128, 32, 32, 4, 2, 1),
            nn.LeakyReLU(0.2),
            ConvTranspose2dWNUB(128, 64, 64, 64, 4, 2, 1),
            nn.LeakyReLU(0.2),
            # ConvTranspose2dWNUB(64, 64, 128, 128, 4, 2, 1),
            # nn.LeakyReLU(0.2),
            # ConvTranspose2dWNUB(64, 1, 256, 256, 4, 2, 1),
            ConvTranspose2dWNUB(64, 1, 128, 128, 4, 2, 1),
        )
        self.beta = beta
        self.apply(lambda x: glorot(x, 0.2))
        glorot(self.conv_block[-1], 1.0)
