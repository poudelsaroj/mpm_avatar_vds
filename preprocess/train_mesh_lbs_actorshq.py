...
def get_dataset(t, md, seq,  data_path, downsample=False, downsample_view=1):
    dataset = []
    if os.path.exists(f'../data/{seq}/smplx_fitted/{str(t).zfill(6)}.pth'):
        smplx_param_t = torch.load(f'../data/{seq}/smplx_fitted/{str(t).zfill(6)}.pth')
    else:
        smplx_param_t = torch.load(f'../data/{seq}/smplx_fitted/{str(t).zfill(6)}/smplx_icp_param.pth')
    smplx_param_t = {k: torch.tensor(v).cuda() for k, v in smplx_param_t.items()}
    if os.path.exists(f'../data/{seq}/smplx_fitted/{str(t + 1).zfill(6)}.pth'):
        smplx_param_t_1 = torch.load(f'../data/{seq}/smplx_fitted/{str(t + 1).zfill(6)}.pth')
    else:
        smplx_param_t_1 = torch.load(f'../data/{seq}/smplx_fitted/{str(t + 1).zfill(6)}/smplx_icp_param.pth')
    smplx_param_t_1 = {k: torch.tensor(v).cuda() for k, v in smplx_param_t_1.items()}
    if os.path.exists(f'../data/{args.seq}/smplx_fitted/{str(t).zfill(6)}.obj'):
        smplx_v, smplx_f, smplx_vn = read_obj(f'../data/{args.seq}/smplx_fitted/{str(t).zfill(6)}.obj')
    else:
        smplx_v, smplx_f, smplx_vn = read_obj(f'../data/{args.seq}/smplx_fitted/{str(t).zfill(6)}/smplx_icp.obj')
    ...
    for i, c in enumerate(md.keys()):
        ...
        msk_name = str(t).zfill(6) + ".png"

        im = np.array(copy.deepcopy(Image.open(os.path.join(data_path, f"4x/rgbs/Cam{cam_name}/Cam{cam_name}_rgb{im_name}")))).astype(np.float32) / 255.
        msk = np.array(copy.deepcopy(Image.open(os.path.join(data_path, f"4x/masks/Cam{cam_name}/Cam{cam_name}_mask{msk_name}")))).astype(np.float32) / 255.
        im = im * msk[..., None]
        ...
        dataset.append({'cam': cam, 'im': im, 'msk': msk, 'id': int(cam_name) - 1, 't': t,
...
def get_loss(params, curr_data, variables, is_initial_timestep, args):
    ...
    im, _, _, msk, radius, _ = Renderer(raster_settings=curr_data['cam'])(**rendervar)
    ...
    loss_weights = {'im': args.img_weight, 'rigid': 0.0, 'rot': .0, 'iso': args.iso_weight, 'floor': 0.0,
                    ...
                    'smplx_dis': args.smplx_dis_weight, 'msk': 1}
...
def report_progress(params, variables, data, i, progress_bar, num_iter_per_timestep, every_i=100):
    if i % every_i == 0 or i == num_iter_per_timestep-1:
        im, _, _, msk, _, _ = Renderer(raster_settings=data['cam'])(**params2rendervar(params, variables))
        curr_id = data['id']
        im = torch.exp(params['cam_m'][curr_id])[:, None, None] * im + params['cam_c'][curr_id][:, None, None]
...
def train(seq, args):
    ...
    lbs_deformer = SmplxDeformer(model_path="../data/body_models", gender=args.smplx_gender, num_betas=300, use_pca=False)
    ...
    for t in range(resume_idx, resume_idx + num_timesteps):
        ...
        if is_initial_timestep:
            ...
            optimizer_smplx = torch.optim.Adam([v for k, v in smplx_param_t.items() if v.requires_grad], lr=args.lr_smplx)
        if os.path.exists('../data/{}/smplx_fitted/{}.obj'.format(seq, str(t).zfill(6))):
            _, smplx_f, _ = read_obj('../data/{}/smplx_fitted/{}.obj'.format(seq, str(t).zfill(6)))
        else:
            _, smplx_f, _ = read_obj('../data/{}/smplx_fitted/{}/smplx_icp.obj'.format(seq, str(t).zfill(6)))
        smplx_f = torch.from_numpy(smplx_f).cuda().long()
        ...
        for i in range(num_iter_per_timestep):
            ...
            loss.backward()
            with torch.no_grad():
                report_progress(params, variables, dataset[6], i, progress_bar, num_iter_per_timestep)
